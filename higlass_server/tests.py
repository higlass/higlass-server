...
class CommandlineTest(unittest.TestCase):
    ...
    def test_cli_upload(self):
        ...
        id = "cli-test"
        self.assertRun(
            "python manage.py ingest_tileset --filename data/"
            + cooler
            + " --datatype matrix --filetype cooler --uid "
            + id
            + " --settings="
            + settings
        )
        self.assertRun(
            "curl -s http://localhost:6000/api/v1/tileset_info/?d=" + id,
            [r'"name": "' + cooler + '"'],
        )
        self.assertRun(
            "curl -s http://localhost:6000/api/v1/tiles/?d=" + id + ".1.1.1",
            [
                r'"' + id + '.1.1.1":',
