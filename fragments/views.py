...
GET_FRAG_PARAMS = {
    ...
    "no-normalize": {
        ...
        "default": False,
        "help": (
            "Do not normalize fragments if true. " "(Only used for cooler tilesets.)"
        ),
    },
    "aggregate": {
        "short": "ag",
    ...
    "representatives": {
        ...
        "default": 0,
        "help": (
            "Number of representative fragments when requesting multiple " "fragments."
        ),
    },
}
...
def get_fragments_by_loci(request):
    ...
    try:
        ...
        return JsonResponse(
            {"error": "Could not read request body.", "error_message": str(e)},
            status=400,
        )
    ...
    *) Optional
    ...
    try:
        for locus in loci:
            ...
            if locus[tileset_idx]:
                ...
                return JsonResponse({"error": "Tileset not specified"}, status=400)

            # Get the dimensions of the snippets (i.e., width and height in px)
            inset_dim = (
                locus[zoom_level_idx + 1]
                if (len(locus) >= zoom_level_idx + 2 and locus[zoom_level_idx + 1])
                else None
            )
            ...
            if filetype != new_filetype:
                return JsonResponse(
                    {"error": ("Multiple file types per query are not supported yet.")},
                    status=400,
                )
        ...
        return JsonResponse(
            {"error": "Could not convert loci.", "error_message": str(e)}, status=500
        )
    ...
    try:
        ...
        raise
        return JsonResponse(
            {"error": "Could not retrieve fragments.", "error_message": str(ex)},
            status=500,
        )
    ...
    if aggregate and len(matrices) > 1:
        try:
            cover, previews_1d, previews_2d = aggregate_frags(
                matrices, loci_ids, aggregation_method, max_previews
            )
            matrices = [cover]
            mat_idx = []
            if previews_1d is not None:
                previews = np.split(previews_1d, range(1, previews_1d.shape[0]))
            data_types = [data_types[0]]
        except Exception as ex:
            raise
            return JsonResponse(
                {"error": "Could not aggregate fragments.", "error_message": str(ex)},
                status=500,
            )
    ...
    if encoding == "b64":
        ...
        if max_previews > 0:
            for i, preview in enumerate(previews):
                previews[i] = pybase64.b64encode(np_to_png(preview)).decode("ascii")
            for i, preview_2d in enumerate(previews_2d):
                previews_2d[i] = pybase64.b64encode(np_to_png(preview_2d)).decode(
                    "ascii"
                )
...
def fragments_by_chr(request):
    ...
    if cooler_file:
        if cooler_file.endswith(".cool"):
            ...
            try:
                ...
                return JsonResponse(
                    {"error": "Cooler file not in database"}, status=500
                )
    else:
        return JsonResponse({"error": "Cooler file not specified"}, status=500)
    ...
    try:
        ...
        return JsonResponse(
            {"error": "Could not retrieve loci.", "error_message": str(e)}, status=500
        )
    ...
    try:
        matrices = get_frag_by_loc(
            cooler_file, loci_rel_chroms, zoomout_level=zoomout_level
        )
    except Exception as e:
        return JsonResponse(
            {"error": "Could not retrieve fragments.", "error_message": str(e)},
            status=500,
        )
    ...
    for matrix in matrices:
        ...
        for measure in measures:
            if measure == "distance-to-diagonal":
                measures_values.append(calc_measure_dtd(matrix, loci_struct[i]))

            if measure == "size":
                measures_values.append(calc_measure_size(matrix, loci_struct[i]))
