...
def ingest(
    ...
    **ignored,
    ...
    if filetype == "bam":
        if indexfile is None:
            ...
            if not op.exists(indexfile):
                raise CommandError(
                    f"Index for bamfile {indexfile} not found. "
    ...
    if no_upload:
        if (
            not op.isfile(op.join(settings.MEDIA_ROOT, filename))
            and not op.islink(op.join(settings.MEDIA_ROOT, filename))
            and not any(
                [
        ...
        if indexfile:
            if (
                not op.isfile(op.join(settings.MEDIA_ROOT, indexfile))
                and not op.islink(op.join(settings.MEDIA_ROOT, indexfile))
                and not any(
                    [
    ...
    try:
        ...
        project_obj = tm.Project.objects.create(name=project_name)
...
def check_for_chromsizes(filename, coord_system):
    ...
    tsinfo_chromsizes = set(
        [(str(chrom), str(size)) for chrom, size in tileset_info["chromsizes"]]
    )
    ...
    if coord_system is not None and len(coord_system) > 0:
        try:
            chrom_info_tileset = tm.Tileset.objects.filter(
                coordSystem=coord_system, datatype="chromsizes"
            )

            if len(chrom_info_tileset) > 1:
                raise CommandError(
                    "More than one available set of chromSizes"
    ...
    if chrom_info_tileset is None:
        ...
        for chrom_info_tileset in tm.Tileset.objects.filter(datatype="chromsizes"):
            chromsizes_set = set(
                [
                    tuple(t)
                    for t in tcs.get_tsv_chromsizes(chrom_info_tileset.datafile.path)
                ]
            )

            matches += [
                (
                    len(set.intersection(chromsizes_set, tsinfo_chromsizes)),
                    chrom_info_tileset,
                )
            ]
        ...
        chromsizes_set = set(
            [tuple(t) for t in tcs.get_tsv_chromsizes(chrom_info_tileset.datafile.path)]
        )
        matches += [
            (
                len(set.intersection(chromsizes_set, tsinfo_chromsizes)),
    ...
    if len(overlap_matches) == 0:
        raise CommandError(
            "No chromsizes available which match the chromosomes in this bigwig"
    ...
    if len(overlap_matches) > 1:
        raise CommandError(
            "Multiple matching coordSystems:"
            + "See http://docs.higlass.io/data_preparation.html#bigwig-files "
            + "for more information",
    ...
    if (
        ...
        raise CommandError(
            f"Matching chromosome sizes (coordSystem: {overlap_matches[0][1].coordSystem}) do not "
            + f"match the specified coordinate sytem ({coord_system}). "
    ...
    if coord_system is None or len(coord_system) == 0:
        print(
            "No coordinate system specified, but we found matching "
            + "chromsizes. Using coordinate system {}.".format(
                overlap_matches[0][1].coordSystem
            )
        )
