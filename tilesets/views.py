...
def tiles(request):
    ...
    accessible_tilesets = [
        ...
        for t in tilesets
        if ((not t.private) or request.user == t.owner)
    ]
...
def link_tile(request):
    ...
    if abs_filepath.find(media_base_path) != 0:
        ...
        return JsonResponse(
            {
                "error": "Provided path ({}) not in the data path".format(
                    body["filepath"]
                )
            },
            status=422,
        ...
        if not op.exists(abs_filepath):
            return JsonResponse(
                {
                    "error": "Specified file ({}) does not exist".format(
                        body["filepath"]
                    )
                },
                status=400,
...
class TilesetsViewSet(viewsets.ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if not uuid:
            return JsonResponse(
                {"error": "The uuid parameter is undefined"}, status=400
            )
        try:
            ...
            if not op.isfile(filepath):
                return JsonResponse(
                    {
                        "error": "Unable to locate tileset media file for deletion: {}".format(
                            filepath
                        )
                    },
                    status=500,
            ...
            return JsonResponse(
                {
                    "error": "Unable to locate tileset instance for uuid: {}".format(
                        uuid
                    )
                },
                status=404,
            ...
            return JsonResponse(
                {"error": "Unable to delete tileset instance: {}".format(str(dbpe))},
                status=500,
            )
        except OSError:
            return JsonResponse(
                {"error": "Unable to delete tileset media file: {}".format(filepath)},
                status=500,
            )
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        try:
            ...
            return JsonResponse(
                {"error": "Unable to retrieve tileset instance: {}".format(str(dne))},
                status=500,
            )
        serializer = tss.UserFacingTilesetSerializer(queryset, many=True)
        try:
    ...
    def perform_create(self, serializer):
        ...
        if self.request.user.is_anonymous:
            ...
            serializer.save(
                owner=gu.get_anonymous_user(), private=False, name=name, uuid=uid
            )
        else:
