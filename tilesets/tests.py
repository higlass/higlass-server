...
def add_file(filename, sub_dir="uploads/data"):
    ...
    def test_list_tilesets(self):
        user1 = dcam.User.objects.create_user(username="user1", password="pass")
        ...
        ret = self.client.get("/api/v1/list_tilesets/?")
        val = json.loads(ret.content)
        ...
        ret = self.client.get("/api/v1/list_tilesets/?ac=test")
        val = json.loads(ret.content)
        ...
        ret = self.client.get("/api/v1/list_tilesets/?ac=user1")
        val = json.loads(ret.content)
    ...
    def test_create(self):
        ...
        ts1 = tm.Tileset.objects.create(name="blah")
...
class TileTests(dt.TestCase):
    def test_partitioning(self):
        ...
        result = tgt.partition_by_adjacent_tiles(
            ["a.5.0.0", "a.5.0.10", "a.5.0.11", "a.7.11"]
        )
...
class BamTests(dt.TestCase):
    def test_get_tile(self):
        ...
        mv = tm.Tileset.objects.create(
            datafile=dcfu.SimpleUploadedFile(bam_file.name, bam_file.read()),
            indexfile=dcfu.SimpleUploadedFile(bai_file.name, bai_file.read()),
            filetype="bam",
            datatype="reads",
            coordSystem="unknown",
            owner=self.user1,
    ...
    def test_register_bam_url(self):
        ...
        record_matches = [
            result for result in response.json()["results"] if result["uuid"] == uid
        ]
...
class ChromosomeSizes(dt.TestCase):
    def test_list_chromsizes(self):
        ...
        ret = json.loads(
            self.client.get("/api/v1/available-chrom-sizes/").content.decode("utf-8")
        )
        ...
        ret = self.client.get("/api/v1/chrom-sizes/?id=cs-hg19&type=json&cum=1")
    ...
    def test_chromsizes_from_cooler(self):
        self.user1 = dcam.User.objects.create_user(username="user1", password="pass")
        upload_file = open(
            "data/dixon2012-h1hesc-hindiii-allreps-filtered.1000kb.multires.cool", "rb"
        )
        self.chroms = tm.Tileset.objects.create(
            datafile=dcfu.SimpleUploadedFile(upload_file.name, upload_file.read()),
            filetype="cooler",
        ...
        ret = self.client.get("/api/v1/chrom-sizes/?id=cooler-dixon&type=json&cum=1")
...
class ViewConfTest(dt.TestCase):
    def setUp(self):
        ...
        self.viewconf = tm.ViewConf.objects.create(viewconf=upload_json_text, uuid="md")
    ...
    def test_duplicate_uid_errors(self):
        ...
        self.assertEqual(ret1.status_code, 200 if hss.UPLOAD_ENABLED else 403)
...
class PermissionsTest(dt.TestCase):
    def setUp(self):
        ...
        self.user2 = dcam.User.objects.create_user(username="user2", password="pass")
    ...
    def test_permissions(self):
        ...
        response = c1.post("/api/v1/tilesets/", test_tileset, format="multipart")
...
class BigWigTest(dt.TestCase):
    def setUp(self):
        ...
        upload_file = open(
            "data/wgEncodeCaltechRnaSeqHuvecR1x75dTh1014IlnaPlusSignalRep2.bigWig", "rb"
        )
        # x = upload_file.read()
    ...
    def test_get_tiles(self):
        ...
        # the assembly
        ret = json.loads(
            c1.get("/api/v1/tiles/?d=bw.22.4194303").content.decode("utf-8")
        )
...
class BigBedTest(dt.TestCase):
    ...
    def test_get_tileset_info(self):
        ...
        try:
            ret = json.loads(
                c1.get("/api/v1/tileset_info/?d=bb").content.decode("utf-8")
            )
    ...
    def test_get_tiles(self):
        ...
        try:
            ret = json.loads(
                c1.get("/api/v1/tiles/?d=bb.14.13").content.decode("utf-8")
            )
            assert len(ret) == 1
        except OSError:
            pass
...
class CoolerTest(dt.TestCase):
    def setUp(self):
        ...
        upload_file = open(
            "data/Dixon2012-J1-NcoI-R1-filtered.100kb.multires.cool", "rb"
        )
    ...
    def test_transforms(self):
        ...
        assert (
            contents["md.0.0.0.default"]["dense"] != contents1["md.0.0.0.none"]["dense"]
        )
    ...
    def test_get_multi_tiles(self):
        ret = self.client.get(
            "/api/v1/tiles/?d=md.7.92.97&d=md.7.92.98&d=md.7.93.97&d=md.7.93.98&d=md.7.93.21"
        )
        content = json.loads(ret.content.decode("utf-8"))
...
class Bed2DDBTest(dt.TestCase):
    def setUp(self):
        ...
        upload_file1 = open(
            "data/Rao_RepH_GM12878_InsulationScore.txt.multires.db", "rb"
        )
    ...
    def test_uids_by_filename(self):
        ret = self.client.get(
            "/api/v1/uids_by_filename/?d=Rao_RepH_GM12878_InsulationScore.txt"
        )
        ...
        ret = self.client.get(
            "/api/v1/uids_by_filename/?d=xRao_RepH_GM12878_InsulationScore"
        )
    ...
    def test_get_tile(self):
        ...
        returned_text = self.client.get(
            "/api/v1/tiles/?d={tile_id}".format(tile_id=tile_id)
        )
    ...
    def test_get_tiles(self):
        ...
        returned_text = self.client.get(
            "/api/v1/tiles/?d={}&d={}&d={}".format(tile_id00, tile_id01, tile_id10)
        )
        returned = json.loads(returned_text.content.decode("utf-8"))
...
class BedDBTest(dt.TestCase):
    ...
    def test_get_tiles(self):
        ...
        returned_text = self.client.get(
            "/api/v1/tiles/?d={tile_id}&d={tile_id1}".format(
                tile_id=tile_id, tile_id1=tile_id1
            )
        )
        returned = json.loads(returned_text.content.decode("utf-8"))
...
class HiBedTest(dt.TestCase):
    ...
    def test_hibed_get_tileset_info(self):
        ...
        returned_text = self.client.get(
            "/api/v1/tileset_info/?d={tile_id}".format(tile_id=tile_id)
        )
        returned = json.loads(returned_text.content.decode("utf-8"))
...
class TilesetsViewSetTest(dt.TestCase):
    def setUp(self):
        ...
        upload_file = open(
            "data/wgEncodeCaltechRnaSeqHuvecR1x75dTh1014IlnaPlusSignalRep2.hitile", "rb"
        )
    ...
    def check_tile(self, z, x, y):
        ...
        r = base64.decodestring(
            returned[list(returned.keys())[0]]["dense"].encode("utf-8")
        )
        q = np.frombuffer(r, dtype=np.float32)
    ...
    def test_get_many_tiles(self):
        ...
        self.assertTrue("{uuid}.1.0.0".format(uuid=self.cooler.uuid) in returned.keys())
        self.assertTrue("{uuid}.1.0.1".format(uuid=self.cooler.uuid) in returned.keys())
    ...
    def test_get_nonexistent_tile(self):
        ...
        self.assertTrue(
            "{uuid}.1.5.5".format(uuid=self.cooler.uuid) not in returned.keys()
        )
    ...
    def test_get_hitile_tileset_info(self):
        ...
        self.assertEqual(returned[uuid]["max_zoom"], 22)
        self.assertEqual(returned[uuid]["max_width"], 2 ** 32)

        self.assertTrue("name" in returned[uuid])
    ...
    def test_list_tilesets(self):
        ...
        if hss.UPLOAD_ENABLED:
            ...
            ret = json.loads(
                c.get("/api/v1/tilesets/?ac=ne&t=cooler").content.decode("utf-8")
            )
            count1 = ret["count"]
            self.assertTrue(count1 == 0)
    ...
    def test_add_with_uid(self):
        ...
        if hss.UPLOAD_ENABLED:
            ...
            ret = json.loads(
                self.client.get("/api/v1/tilesets/").content.decode("utf-8")
            )
            self.assertEquals(ret["count"], 3)
        else:
    ...
    def test_list_by_datatype(self):
        ...
        ret = json.loads(
            self.client.get("/api/v1/tilesets/?dt=1").content.decode("utf-8")
        )
        self.assertEqual(ret["count"], 1 if hss.UPLOAD_ENABLED else 0)

        ret = json.loads(
            self.client.get("/api/v1/tilesets/?dt=2").content.decode("utf-8")
        )
        self.assertEqual(ret["count"], 1 if hss.UPLOAD_ENABLED else 0)

        ret = json.loads(
            self.client.get("/api/v1/tilesets/?dt=1&dt=2").content.decode("utf-8")
        )
        self.assertEqual(ret["count"], 2 if hss.UPLOAD_ENABLED else 0)

    def test_get_nonexistant_tileset_info(self):
        ret = json.loads(
            self.client.get("/api/v1/tileset_info/?d=x1x").content.decode("utf-8")
        )
